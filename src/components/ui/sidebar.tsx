        ...
        <div
          className={cn(
            ...
            side === "left"
              ? "left-0"
              : "right-0",
            collapsible === "offcanvas" && side === 'left' && "group-data-[state=collapsed]/sidebar:-left-[--sidebar-width]",
            collapsible === "offcanvas" && side === 'right' && "group-data-[state=collapsed]/sidebar:-right-[--sidebar-width]",
...
>(({ className, ...props }, ref) => {
  ...
  const handleContentClick = (event: React.MouseEvent) => {
    if (open && !isMobile) {
      if ((event.target as HTMLElement).closest('[data-sidebar="sidebar"]')) {
        return;
      }
      toggleSidebar();
    }
  ...
  return (
    <main
      ref={ref}
      className={cn(
        "flex min-h-svh flex-1 flex-col bg-background transition-[margin-left] duration-200 ease-linear overflow-x-hidden",
        "md:ml-[--sidebar-width]",
        "md:peer-data-[collapsible=icon]:ml-[--sidebar-width-icon]",
        "md:group-data-[state=collapsed]/sidebar-wrapper:ml-0",
        className
      )}
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 rounded-md p-2 text-left text-sm outline-none ring-ring transition-[width,height,padding] hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 active:bg-accent active:text-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-accent data-[active=true]:font-medium data-[active=true]:text-accent-foreground data-[state=open]:hover:bg-accent data-[state=open]:hover:text-accent-foreground group-data-[collapsible=icon]:!p-2 group-data-[collapsible=icon]:w-8 group-data-[collapsible=icon]:overflow-hidden [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
